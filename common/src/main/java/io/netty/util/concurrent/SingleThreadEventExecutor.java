 ...
 *
 */
//于此对应的是MultithreadEventExecutorGroup
//只有个线程的线程池 有很多的任务(与IO线程不一样)
public abstract class SingleThreadEventExecutor extends AbstractScheduledEventExecutor implements OrderedEventExecutor {
     ...
     */
    //调用线程调用 不是线程池里面的任务线程调用?

    //在内部实现中，到了触发时间的scheduledTask会被放入taskQueue，
    // 所以我们看到返回的总是taskQeue内的task。 因为是个死循环 到时间的任务放入TaskQueue中
    //后 很快的进入下次循环获取到任务执行 这个技巧可以在面试中说
        ...
        for (;;) {
            ...
            if (scheduledTask == null) {
                ...
                if (task == null) {
                    ...
                    //这个方法是必然把ScheduledTaskQueue调度队列中的任务加入到 TaskQueue中
                    // 这是添加到任务队列中 然后再取出
    ...
    protected void wakeup(boolean inEventLoop) {
        if (!inEventLoop) {
            ...
            //offer是有true和false false的话证明队列中有元素 则无所谓
            //为了唤醒在阻塞队列中的线程
    ...
    //线程池必须得有线程池内的线程关闭
    //要求EventExecutor将队列内的任务全部执行完（未触发的调度任务不执行）
    // ，以及注册的ShutDownHook全部执行完，
    //只是更改线程池的状态 更改成功与否是根据状态来彻底的关闭
    @Override
     ...
     */
    //真正关闭线程池 尝试关闭线程池调用多次
    //返回boolean值 是关闭任务是在别的地方关闭吗 这个只是判断一下
    protected boolean confirmShutdown() {
        //必须已经进ST_SHUTTING_DOWN以上的状态 向下执行
        if (!isShuttingDown()) {
            return false;
        }
    ...
    //这都不是线程池内部的线程调用 是谁调用用线程池的线程调用
    private void execute(Runnable task, boolean immediate) {
        boolean inEventLoop = inEventLoop();
        //添加Task 内部线程也可以添加到队列中 添加到队列中就相当于把任务提交给线程池
        addTask(task);
        // 启动的时候必须是外部线程调用任务
        if (!inEventLoop) {
            startThread();
        ...
        //这个逻辑与线程启动无关，它的意思是：addTask无法自动唤醒Executor线程，
        // 那么调用wakeup方法来唤醒

        //因为SingleThreadEventExecutor.run方法是由子类实现的，那么线程到底会如何阻塞，
        // 如何唤醒阻塞，只有子类才知道。如果run实现只会阻塞在taskQueue上（addTaskWakesUp=true），
        // 此时上面的wakeup方法确实可以唤醒阻塞线程。否则的话，addTaskWakesUp=false，
        // 同时子类要重写wakeup方法实现自己的唤醒逻辑。
    ...
    private void startThread() {
        ...
        if (state == ST_NOT_STARTED) {
            //防止多线程 cas 并且 如果是失败后则线程已经启动直接返回就行
            if (STATE_UPDATER.compareAndSet(this, ST_NOT_STARTED, ST_STARTED)) {
                ...
                try {
                    ...
                    //这是一个 技巧 埋点如果执行到success的赋值 则在此复制之前没有发生错误
                    if (!success) {
    ...
    private void doStartThread() {
        assert thread == null;
        //这个任务 是为了获取任务队列的任务然后执行
        //这个只是执行一次
        executor.execute(new Runnable() {
            @Override
            public void run() {

                //本EventExecutor 获取到线程 标记一下线程已经启动 线程池已经启动
